# input 1 : #minecraft:#mcr_custom_sandstone_chiseled_sandstone_cut_sandstone
...
function mcr-autocraft:count/cut_sandstone
...
# try to add items
execute if score @s mcrac_ok matches 1 store result score @s mcrac_gave run loot insert ^ ^ ^-1 loot mcr-autocraft:sandstone_stairs_4
...
execute if score @s mcrac_ok matches 1 if score @s mcrac_gave matches 1 if score @s mcrac_ic_1_t3_d matches 1 store result block ^ ^ ^-1 Items[{id:"minecraft:cut_sandstone"}].Count short 1 run scoreboard players get @s mcrac_ic_1_t3
