# input 1 : minecraft:end_stone_bricks
...
# calculate costs
function mcr-autocraft:count/end_stone_bricks
...
# try to add items
execute if score @s mcrac_ok matches 1 store result score @s mcrac_gave run loot insert ^ ^ ^-1 loot mcr-autocraft:end_stone_brick_stairs_4

# if ok, remove crafting items
execute if score @s mcrac_ok matches 1 if score @s mcrac_gave matches 1 if score @s mcrac_ic_1_t1_d matches 1 store result block ^ ^ ^-1 Items[{id:"minecraft:end_stone_bricks"}].Count short 1 run scoreboard players get @s mcrac_ic_1_t1
