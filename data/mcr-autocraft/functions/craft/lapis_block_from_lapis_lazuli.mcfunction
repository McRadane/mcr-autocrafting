# input 1 : minecraft:lapis_lazuli
...
# calculate costs
function mcr-autocraft:count/lapis_lazuli
...
# try to add items
execute if score @s mcrac_ok matches 1 store result score @s mcrac_gave run loot insert ^ ^ ^-1 loot mcr-autocraft:lapis_block_1

# if ok, remove crafting items
execute if score @s mcrac_ok matches 1 if score @s mcrac_gave matches 1 if score @s mcrac_ic_1_t1_d matches 1 store result block ^ ^ ^-1 Items[{id:"minecraft:lapis_lazuli"}].Count short 1 run scoreboard players get @s mcrac_ic_1_t1
